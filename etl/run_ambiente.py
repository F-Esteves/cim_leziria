...
def correr_passo(nome: str):
    ...
    if nome == "extract":
        from etl.extract_ambiente import main
    elif nome == "transform":
        from etl.transform_ambiente import main
    elif nome == "validate":
        from etl.validate_ambiente import main
    elif nome == "load":
        from etl.load_ambiente import main
    else:
        print(f"  Passo desconhecido: {nome}"); return False
...
def main():
    ...
    print(f"\n{'='*55}")
    print(f"  CIM Lezíria do Tejo · Cluster 2 — Ambiente")
