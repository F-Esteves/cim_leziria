...
def main() -> None:
    ...
    if not path.exists():
        print("  ✗ mob_transformed.parquet não encontrado — corre transform primeiro")
        return
    ...
    mun_map, met_map, tmp_map = carregar_lookup(conn, "Mobilidade")
    ...
    if metricas_sem_id:
        ...
        for m in sorted(metricas_sem_id):
            print(f"     · {m}")
        print("     → Correr schema_mobilidade.sql antes deste script!\n")
    ...
    print("\n[ fact_scores ]")
    calcular_scores_sql(conn, "Mobilidade")
