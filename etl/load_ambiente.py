...
def load_indicadores(conn, df: pd.DataFrame):
    ...
    if sem_mun: print(f"  ⚠ Municípios sem ID: {sem_mun}")
    if sem_met: print(f"  ⚠ Métricas sem ID:   {sem_met}")
    cur.close()
...
def calcular_scores(conn):
    ...
    cur.execute("""
        ...
        WHERE me.cluster = 'Ambiente' AND f.valor_normalizado IS NOT NULL
...
def main():
    print("\n=== LOAD · Cluster 2 — Ambiente ===\n")

    path = STAGING_DIR / "amb_transformed.parquet"
    if not path.exists():
        print("  ✗ amb_transformed.parquet não encontrado"); return
